...
export function renderFlowchartAscii(source: string): { lines: string[]; error?: string; warnings?: string[] } {
  ...
  try {
    ...
    if (ascii.includes('flowchart ') || ascii.includes('graph ') ||
        ascii.match(/\w+\s*-->\s*\w+/) || ascii.match(/\w+\s*---\s*\w+/)) {
      throw new Error('mermaid-ascii could not render this diagram - showing source view');
    }
...
export function renderSequenceAscii(source: string): { lines: string[]; error?: string } {
  try {
...
export function renderClassAscii(source: string): { lines: string[]; error?: string } {
  try {
...
export function calculateMinimumSize(width: number, height: number): SizeModifier {
