...
import { ScrollBar, useMouseScroll, useTerminalSize } from "./shared/index.js";
...
export default function Checklist() {
          ...
          return parsed.map((entry) => {
            ...
            if (entry && typeof entry === "object") {
              ...
              const checked = Boolean(
                obj.checked ?? obj.completed ?? obj.selected
              );
  ...
  useInput((input, key) => {
    ...
    if (key.downArrow) {
      setCursor((c) => {
        ...
        if (newC >= scroll + visibleLines)
          setScroll(Math.min(maxScroll, newC - visibleLines + 1));
        return newC;
      });
      return;
    }
  ...
  return (
    <Box flexDirection="column" paddingX={1}>
      {!args?.["no-header"] && (
        <Box marginBottom={1}>
          ...
          {showScrollBar && (
            <Text dimColor>
              {" "}
              ({scroll + 1}-{Math.min(scroll + visibleLines, items.length)}/
              {items.length})
            </Text>
          )}
        </Box>
      ...
      <Box marginTop={1}>
        <Text dimColor>
          Space=toggle a=all n=none ↑↓=nav Enter=done q=cancel
        </Text>
