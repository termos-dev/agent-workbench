...
import React, { useState, useCallback, useMemo, useEffect, Component, type ReactNode, type ErrorInfo } from 'react';
import { Box, Text, useInput, useApp } from 'ink';
import * as fs from 'fs';
import { useTerminalSize } from '../shared/index.js';
import { useDashboardData, type InteractionResponse } from './use-dashboard-data.js';
import { useAgents, type AgentDisplayStatus } from './use-agents.js';
...
class ErrorBoundary extends Component<{ children: ReactNode; fallback?: ReactNode; onError?: (error: Error) => void }, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo): void {
    ...
    try {
      ...
      fs.appendFileSync('/tmp/termos-crash.log', `${new Date().toISOString()} ${msg}\n`);
    } catch {}
...
const getGlobalArgs = (): DashboardArgs => (globalThis as Record<string, unknown>).args as DashboardArgs || {};
...
export default function Dashboard(props?: DashboardProps) {
  ...
  const handleNew = useCallback((i: DashboardInteraction) => {
    if (process.platform === 'darwin' && props?.onAutoFocus) props.onAutoFocus();
    showStatus(`New: ${i.title || i.component}`);
  }, [props, showStatus]);

  const { allInteractions, loading, error, refresh, respondToInteraction, sendMessage } = useDashboardData({
    refreshInterval: 1000,
    onNewInteraction: handleNew,
  });

  const { agents } = useAgents({ refreshInterval: 1000, interactions: allInteractions });

  // Filter by current project in local mode
  const projectFilter = !args.global ? args.currentProject : null;
  const interactions = useMemo(() => projectFilter ? allInteractions.filter(i => i.project === projectFilter) : allInteractions, [allInteractions, projectFilter]);
  const filteredAgents = useMemo(() => projectFilter ? agents.filter(a => a.project === projectFilter) : agents, [agents, projectFilter]);

  // Group interactions by agent
  const { agentRows, orphans } = useMemo(() => {
    const rows: Array<{ agent: AgentDisplayStatus; interactions: DashboardInteraction[] }> = [];
    const sessionIds = new Set(filteredAgents.map(a => a.sessionId));

    for (const agent of filteredAgents) {
      rows.push({ agent, interactions: interactions.filter(i => (i as { agentSessionId?: string }).agentSessionId === agent.sessionId) });
    }
  ...
  useEffect(() => { if (selectedIdx > maxIdx) setSelectedIdx(Math.max(0, maxIdx)); }, [selectedIdx, maxIdx]);

  const handleRespond = useCallback(async (session: string, id: string, response: InteractionResponse) => {
    try {
      await respondToInteraction(session, id, response);
      showStatus('Responded');
      if (selectedIdx >= interactions.length - 1) setSelectedIdx(Math.max(0, selectedIdx - 1));
    } catch (e) {
      showStatus(`Failed: ${e instanceof Error ? e.message : String(e)}`);
    }
  }, [selectedIdx, interactions.length, respondToInteraction, showStatus]);

  const handleSendMessage = useCallback(async (sessionName: string, text: string, agentSessionId?: string) => {
    try {
      await sendMessage(sessionName, text, agentSessionId);
      showStatus('Message sent');
    } catch (e) {
      showStatus(`Failed to send: ${e instanceof Error ? e.message : String(e)}`);
    }
  ...
  const focusedAgent = useMemo(() =>
    focusedSessionId ? filteredAgents.find(a => a.sessionId === focusedSessionId) : null,
    [focusedSessionId, filteredAgents]
  );

  const focusedInteractions = useMemo(() =>
    focusedAgent ? interactions.filter(i => (i as { agentSessionId?: string }).agentSessionId === focusedAgent.sessionId) : [],
  ...
  useInput((input, key) => {
    ...
    // Refresh
    if (input === 'r') { refresh(); showStatus('Refreshing...'); return; }
    ...
    // g + g = go to first
    if (input === 'g') {
      if (gPressed) { setSelectedIdx(0); setSelectedAgentIdx(0); setGPressed(false); }
      else { setGPressed(true); setTimeout(() => setGPressed(false), 500); }
      return;
    }
    ...
    // Home/End
    if (key.home) { setSelectedIdx(0); setSelectedAgentIdx(0); return; }
    if (key.end) { setSelectedIdx(maxIdx); setSelectedAgentIdx(maxAgentIdx); return; }

    // Navigation
    const skipArrows = selected?.component === 'ask';
    const askMultiQ = selected?.component === 'ask' && ((selected.args as { schema?: { questions?: unknown[] } })?.schema?.questions?.length || 0) > 1;

    if (key.tab && !askMultiQ) {
      setSelectedIdx(i => key.shift ? (i <= 0 ? maxIdx : i - 1) : (i + 1) % (maxIdx + 1));
      return;
    }

    if (selected?.component !== 'input') {
      if ((key.upArrow && !skipArrows) || input === 'k') {
      ...
      if ((key.downArrow && !skipArrows) || input === 'j') {
  ...
  if (error) return <Box flexDirection="column" padding={1}><Text color="red">{error}</Text><Text dimColor>r retry</Text></Box>;

  const currentPath = !args.global ? process.cwd() : null;
  const activePath = filteredAgents.find(a => a.project === projectFilter)?.projectPath || currentPath;
  ...
  if (agents.length === 0 && allInteractions.length === 0) {
    return (
      <Box flexDirection="column" width={columns} height={rows}>
        ...
        <Box flexGrow={1} />
        {currentPath && <Box paddingX={1}><Text dimColor>{currentPath}</Text></Box>}
        <Box paddingX={1}><Text dimColor>r refresh  ? help</Text></Box>
      </Box>
    );
  }
  ...
  if (focusedSessionId && focusedAgent) {
    ...
    const sessionName = focusedInteractions[0]?.sessionName ||
      (focusedAgent.projectPath ? focusedAgent.projectPath.replace(/[/\\]/g, '-').replace(/^-/, '') : '');
    ...
    return (
      ...
      >
        <FocusedSessionView
          ...
          onSendMessage={sessionName ? (text) => handleSendMessage(sessionName, text, focusedAgent.sessionId) : undefined}
        />
      </ErrorBoundary>
    );
  }
  ...
  const hints = selected ? (
    selected.component === 'confirm' ? 'y/n respond  ' :
    selected.component === 'select' ? 'h/l ←→ select  Enter confirm  ' :
    selected.component === 'ask' ? '↑↓ options  Tab questions  Space select  Enter submit  ' :
    selected.component === 'input' ? 'Enter submit  ' :
    isInteractive(selected.component) ? '' : 'd dismiss  '
  ) : '';
  ...
  return (
    <Box flexDirection="column" width={columns} height={rows}>
      ...
      {status && <Box paddingX={1}><Text color="yellow">{status}</Text></Box>}

      <Box paddingX={1}>
        <Text bold>{filteredAgents.length} Agents</Text>
        {interactions.length > 0 && <Text color="yellow"> • {interactions.length} pending</Text>}
        {activePath && <Text dimColor>  {activePath}</Text>}
      </Box>

      <Box paddingX={1}>
        <Text dimColor>j/k navigate  {hints}{filteredAgents.length > 0 && !selected ? 'Enter focus  ' : ''}? help</Text>
        {gPressed && <Text color="cyan"> [g pressed - press g again for top]</Text>}
      </Box>
    </Box>
  );
}
