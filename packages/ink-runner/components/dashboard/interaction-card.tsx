...
import { parseTableData, TableRenderer, TableError } from '../shared/index.js';
import {
  ConfirmEmbed,
  SelectEmbed,
  InputEmbed,
...
function renderMd(line: string, i: number) {
  if (line.startsWith('### ')) return <Text key={i} color="yellow">{line.slice(4)}</Text>;
  if (line.startsWith('## ')) return <Text key={i} bold color="cyan">{line.slice(3)}</Text>;
  if (line.startsWith('# ')) return <Text key={i} bold color="green">{line.slice(2)}</Text>;
  if (line.startsWith('- ')) return <Text key={i}><Text color="blue">•</Text> {line.slice(2)}</Text>;
...
export function InteractionCard({
  ...
  const isLiveOutput = interaction.component === 'output' && args?.live === true && args?.outputFile;
  ...
  const content = useMemo(() => {
    if (interaction.component === 'card') {
      ...
      if (c) return c.split('\n');
      if (args?.file) {
        try { return fs.readFileSync(args.file as string, 'utf-8').split('\n'); }
        catch { return [`[Error: ${args.file}]`]; }
      }
    }
    if (interaction.component === 'mermaid') {
      ...
      if (code) source = code;
        ...
        try { source = fs.readFileSync(args.file as string, 'utf-8'); }
        catch { return [`[Error: ${args.file}]`]; }
      }
      if (source) {
        ...
        else if (type === 'state') result = renderStateAscii(clean);
    ...
    return [];
  }, [interaction.component, args, interactive, isLiveOutput]);

  // Table parsing
  const tableResult = useMemo(() => {
    if (interaction.component !== 'table') return null;
    return parseTableData(args?.data as string | unknown[], args?.columns as string);
  ...
  const chartData = useMemo(() => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        return {
          type: 'objects',
          items: parsed.slice(0, 8).map((item: { label?: string; value?: number }, i: number) => ({
    ...
    <Box flexDirection="column" width={width}>
      ...
      {interaction.component !== 'ask' && (
        <Box marginTop={1} marginLeft={2} flexDirection="column">
          {interaction.component === 'card' ? content.map(renderMd) :
           interaction.component === 'mermaid' ? content.map((l, i) => (
             <Text key={i} dimColor={!isSelected}>{l}</Text>
           )) :
             ...
             chartData ? (
               chartType === 'sparkline' && chartData.type === 'numbers' ? (
                 ...
                 <Box flexDirection="column">
                   <Text color="cyan">
                     {chartData.values.map((v: number) => {
                       const min = Math.min(...chartData.values);
                       const max = Math.max(...chartData.values);
                       const range = max - min || 1;
                       const chars = '▁▂▃▄▅▆▇█';
                       const idx = Math.min(Math.floor(((v - min) / range) * 8), 7);
                       return chars[idx];
                     }).join('')}
                   </Text>
                   <Text dimColor>min: {Math.min(...chartData.values)}  max: {Math.max(...chartData.values)}  pts: {chartData.values.length}</Text>
                 </Box>
               ) : chartData.type === 'objects' ? (
                 // Bar chart rendering
                 <Box flexDirection="column">
                   {chartData.items.map((item: { label: string; value: number }, i: number) => {
                     const maxVal = Math.max(...chartData.items.map((d: { value: number }) => d.value), 1);
                     const barWidth = Math.min(30, Math.floor((item.value / maxVal) * 30));
                     const colors = ['cyan', 'green', 'yellow', 'magenta', 'blue'];
                     return (
                       <Box key={i}>
                         <Text dimColor>{item.label}</Text>
                         <Text color={colors[i % colors.length]}>{'█'.repeat(barWidth)}</Text>
                 ...
                 <Box flexDirection="column">
                   {chartData.values.slice(0, 8).map((v: number, i: number) => {
                     ...
                     const barWidth = Math.min(30, Math.floor((v / maxVal) * 30));
                     const colors = ['cyan', 'green', 'yellow', 'magenta', 'blue'];
                     return (
                       <Box key={i}>
           ...
           interaction.component === 'table' && tableResult ? (
             'error' in tableResult ? <TableError error={tableResult.error} /> :
             <TableRenderer data={tableResult.data} width={Math.max(30, width - 10)} maxRows={8} compact />
           ) : isLiveOutput ? (
             <LiveOutputEmbed
               outputFile={args?.outputFile as string}
               isActive={isSelected}
               onRespond={(feedback) => onRespond({ action: 'accept', value: 'dismissed', feedback })}
             />
           ) : !interactive ? content.map((l, i) => <Text key={i} dimColor={!isSelected}>{l}</Text>) :
           <Text>{interaction.prompt || 'Please respond'}</Text>}
        </Box>
      )}

      <Box marginTop={1} marginLeft={2}>
        {interaction.component === 'confirm' && <ConfirmEmbed isActive={isSelected} onRespond={v => onRespond({ action: 'accept', value: v })} onCancel={() => onRespond({ action: 'cancel' })} />}
        {interaction.component === 'select' && <SelectEmbed interaction={interaction} isActive={isSelected} onRespond={v => onRespond({ action: 'accept', value: v })} onCancel={() => onRespond({ action: 'cancel' })} />}
        {interaction.component === 'input' && <InputEmbed interaction={interaction} isActive={isSelected} onRespond={v => onRespond({ action: 'accept', value: v })} onCancel={() => onRespond({ action: 'cancel' })} />}
        {interaction.component === 'ask' && <AskEmbed interaction={interaction} isActive={isSelected} onRespond={a => onRespond({ action: 'accept', answers: a })} onCancel={() => onRespond({ action: 'cancel' })} onTabNext={onTabNext} onTabPrev={onTabPrev} />}
        {interaction.component === 'card' && <CardEmbed interaction={interaction} isActive={isSelected} onRespond={(v, l) => onRespond({ action: 'accept', value: { selected: v, selectedLabel: l } })} onCancel={() => onRespond({ action: 'cancel' })} />}
        {!interactive && !isLiveOutput && isSelected && <DisplayFeedbackEmbed isActive={isSelected} onRespond={(feedback) => onRespond({ action: 'accept', value: 'dismissed', feedback })} />}
      </Box>
    </Box>
  );
}
