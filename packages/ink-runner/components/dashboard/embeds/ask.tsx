...
export function AskEmbed({ interaction, isActive, onRespond, onCancel, onTabNext, onTabPrev }: AskEmbedProps) {
  const schema = (interaction.args as { schema?: { questions: FormQuestion[] } })?.schema;
  const questions = schema?.questions || [];

  const [activeQ, setActiveQ] = useState(0);
  const [cursors, setCursors] = useState<number[]>(() => questions.map(() => 0));
  const [selections, setSelections] = useState<Map<number, Set<string>>>(() => new Map(questions.map((_, i) => [i, new Set()])));
  const [textInputs, setTextInputs] = useState<Map<number, string>>(() => new Map(questions.map((_, i) => [i, ''])));
  const [otherTexts, setOtherTexts] = useState<Map<number, string>>(() => new Map(questions.map((_, i) => [i, ''])));
  ...
  const unanswered = useMemo(() => questions.filter((question, i) => {
    if (question.options) {
      const sel = selections.get(i) || new Set();
  ...
  }).length, [questions, selections, textInputs, otherTexts]);
  ...
  useInput((input, key) => {
    ...
    if (key.escape) {
      if (pendingSkip) { setPendingSkip(false); }
      else { onCancel?.(); }
      return;
    }
    ...
    if (!isText && !isOtherSelected) {
      if (key.upArrow || input === 'k') {
        ...
        if (cursor === 0 && activeQ > 0) {
          ...
          setCursors(p => { const n = [...p]; n[activeQ] = (n[activeQ] - 1 + totalOpts) % totalOpts; return n; });
        }
        return;
      }
      if (key.downArrow || input === 'j') {
        ...
        if (cursor === totalOpts - 1 && activeQ < questions.length - 1) {
          ...
          setCursors(p => { const n = [...p]; n[activeQ] = (n[activeQ] + 1) % totalOpts; return n; });
        }
        return;
      }
      if (input === ' ') {
        ...
        if (label) {
          setSelections(p => {
            ...
            if (q.multiSelect) { s.has(label) ? s.delete(label) : s.add(label); }
            else { s.clear(); s.add(label); }
            m.set(activeQ, s);
            return m;
          });
          // For single-select, clear Other text when selecting a regular option
          if (!q.multiSelect) {
            setOtherTexts(p => { const m = new Map(p); m.set(activeQ, ''); return m; });
          }
        }
        return;
      }
  ...
  const handleTextChange = useCallback((v: string) => {
    setTextInputs(p => { const m = new Map(p); m.set(activeQ, v); return m; });
  }, [activeQ]);
  ...
  }, [activeQ, questions.length, textInputs, canSubmit, buildAnswers, onRespond]);

  const handleOtherTextChange = useCallback((qIdx: number) => (v: string) => {
    setOtherTexts(p => { const m = new Map(p); m.set(qIdx, v); return m; });
  ...
  return (
    <Box flexDirection="column">
      {questions.map((question, i) => {
        ...
        return (
          <Box key={i} flexDirection="column" marginBottom={1}>
            <Text color={active ? 'cyan' : 'gray'} bold>{active ? '> ' : '  '}[{question.header}]</Text>
            <Text color={active ? 'white' : 'gray'}>{active ? '  ' : '  '}{question.question}</Text>
            {qText ? (
              <Box>
                <Text color="green">&gt; </Text>
                {active ? (
                  <TextInput value={txt} onChange={handleTextChange} onSubmit={handleTextSubmit} focus={isActive} placeholder={question.placeholder || 'Type...'} />
                ) : (
                  <Text dimColor>{txt || question.placeholder || 'Type...'}</Text>
                )}
              </Box>
            ) : (
              <Box flexDirection="column">
                {qOpts.map((opt, oi) => (
                  <Text key={oi} color={active && cursor === oi ? 'cyan' : sel.has(opt.label) ? 'green' : active ? 'white' : 'gray'} bold={active && cursor === oi}>
                    {active && cursor === oi ? '> ' : '  '}
                    {question.multiSelect ? (sel.has(opt.label) ? '[x] ' : '[ ] ') : (sel.has(opt.label) ? '● ' : '○ ')}
                    {opt.label}{opt.description && <Text dimColor> - {opt.description}</Text>}
                  </Text>
                ))}
                ...
                {(() => {
                  ...
                  return (
                    <Box>
                      <Text color={active && isCursorOnOther ? 'cyan' : hasOtherText ? 'green' : active ? 'white' : 'gray'} bold={active && isCursorOnOther}>
                        {active && isCursorOnOther ? '> ' : '  '}
                        {question.multiSelect ? (hasOtherText ? '[x] ' : '[ ] ') : (hasOtherText ? '● ' : '○ ')}
                      </Text>
                      {active && isCursorOnOther ? (
                        <TextInput value={otherTextValue} onChange={handleOtherTextChange(i)} onSubmit={handleOtherTextSubmit} focus={isActive} placeholder="Other..." />
                      ) : (
                        <Text dimColor={!hasOtherText} color={hasOtherText ? 'green' : undefined}>{otherTextValue || 'Other...'}</Text>
                      )}
                    </Box>
                  );
                })()}
      ...
      <Text dimColor>↑↓ options  {questions.length > 1 ? 'Tab questions  ' : ''}Space select  {canSubmit ? <Text color="green">Enter submit</Text> : (currentAnswered ? 'Enter next' : 'Enter submit')}  Esc cancel</Text>
      {pendingSkip && !currentAnswered && (
        <Text color="yellow">⚠ {activeQ === questions.length - 1 ? `${unanswered} unanswered. Enter to submit anyway` : 'Enter to skip this question'}</Text>
      )}
    </Box>
  );
}
