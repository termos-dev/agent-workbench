...
import { ScrollBar, useFileWatch, useMouseScroll } from "../shared/index.js";
...
export function PlanPane({
  ...
  const loadContent = useCallback(() => {
    ...
    try {
      ...
      setLines([
        `Error reading plan file: ${e instanceof Error ? e.message : String(e)}`,
      ]);
      setError("read_error");
    }
  }, [validPlanFile]);
  ...
  useInput((input, key) => {
    ...
    if (input === "e" && validPlanFile && !error) {
      ...
      setEditorStatus(
        `Opened in ${editorName}. Switch to editor, save to refresh.`
      );
  ...
  return (
    <Box flexDirection="column" height={height} width={width}>
      ...
      <Box>
        ...
        {editorStatus && <Text color="yellow"> • {editorStatus}</Text>}
      </Box>
