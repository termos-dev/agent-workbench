  ...
  try {
    ...
    return parsed.map((action, idx) => {
      ...
      if (!key || usedKeys.has(key)) {
        ...
        const labelFirstChar = action.label.replace(/[^\w]/g, '')[0]?.toLowerCase();
...
export default function Card() {
  ...
  const isVertical = layout === 'vertical' || (layout === 'auto' && actions.length > 3);
  ...
  useInput((input, key) => {
    ...
    if (hotkeyMatch !== -1) {
      ...
      onComplete({ action: 'accept', selected: action.value, selectedLabel: action.label });
      exit();
      return;
    }
