...
export default function MermaidViewer() {
  ...
  useFileWatch(args?.file, () => {
    try {
      ...
      setError(
        `Error loading diagram: ${e instanceof Error ? e.message : String(e)}`
      );
    }
  });

  const lines =
    viewMode === "ascii" && asciiSupported ? asciiLines : sourceLines;
  ...
  const displayLines = lines.slice(
    verticalScroll,
    verticalScroll + visibleRows
  );
  const verticalScrollPosition =
    maxVerticalScroll > 0 ? verticalScroll / maxVerticalScroll : 0;
    ...
    <Box flexDirection="column">
        ...
        <Box paddingX={1}>
          ...
          {showVerticalScrollBar && (
            <Text dimColor>
              {" "}
              ({verticalScroll + 1}-
              {Math.min(verticalScroll + visibleRows, lines.length)}/
              {lines.length})
            </Text>
          )}
          ...
          {scrolledLines.map((line, idx) => {
            ...
            if (viewMode === "source") {
              ...
              if (line.startsWith("%%")) color = "gray";
                ...
                line.match(/^(flowchart|graph|sequenceDiagram|classDiagram)/i)
              )
                color = "magenta";
              else if (
                line.includes("-->") ||
          ...
          <Text dimColor>
            ...
            {"─".repeat(
              Math.max(
                ...
                visibleCols -
                  6 -
                  Math.min(3, Math.ceil(horizontalScroll / 10)) -
                  Math.min(
                    3,
                    Math.ceil((maxHorizontalScroll - horizontalScroll) / 10)
                  )
              )
            )}
            {"→".repeat(
      ...
      <Box paddingX={1} marginTop={1}>
        <Text dimColor>
          ...
          ↑↓=scroll {showHorizontalScrollIndicator ? "←→/hl=pan  " : ""}
          q=close
