...
export default function TableViewer() {
  ...
  useInput((_input, key) => {
    ...
    if (key.downArrow) {
      if (selectMode && selectedRow !== null) {
        ...
        setSelectedRow(newSel);
        if (newSel >= scroll + visibleRows)
          setScroll(Math.min(maxScroll, newSel - visibleRows + 1));
      } else {
    ...
    if (key.pageDown) {
      ...
      if (selectMode && selectedRow !== null) {
        setSelectedRow((s) =>
          Math.min(rows.length - 1, (s ?? 0) + visibleRows)
        );
      }
    }
  });
  ...
  return (
    <Box flexDirection="column">
      {!args?.["no-header"] && (
        <Box paddingX={1}>
          ...
          {showScrollBar && (
            <Text dimColor>
              {" "}
              [{scroll + 1}-{Math.min(scroll + visibleRows, rows.length)}]
            </Text>
          )}
        </Box>
      )}
      ...
      <Box flexDirection="row">
        ...
        {showScrollBar && (
          <ScrollBar
            ...
            height={Math.min(visibleRows, rows.length) + 3}
          />
        )}
      </Box>
      ...
      <Box paddingX={1}>
        <Text dimColor>
          ↑↓=scroll PgUp/PgDn {selectMode ? "Enter=select  " : ""}q=close
