...
function normalizeLineData(data: unknown): LineGraphData {
  if (Array.isArray(data)) {
    ...
    if ((data[0] as Record<string, unknown>).values) {
      return {
        series: (data as { values: number[]; color?: string }[]).map(
          (s, i) => ({
            values: s.values,
    ...
    return {
      series: [
        {
          values: (data as Record<string, unknown>[]).map((d) =>
            Number(d.value ?? 0)
          ),
          color: "cyan",
        },
  ...
  if (obj.series) {
    return {
      series: (obj.series as { values: number[]; color?: string }[]).map(
        (s, i) => ({
          values: s.values,
          color: s.color || COLORS[i % COLORS.length],
        })
  ...
  return (
    <Box flexDirection="column">
      {items.map((item, i) => {
        ...
        const bar =
          "█".repeat(fullBlocks) + (remainder > 0.1 ? partialChar : "");
...
function LineGraph({
  ...
  maxWidth,
  height,
}: { data: LineGraphData; maxWidth: number; height: number }) {
  ...
  const gridColors: (string | null)[][] = Array(height)
    .fill(null)
  ...
  const yLabels = [max, (max + min) / 2, min].map((v) =>
    v.toFixed(0).padStart(yAxisWidth)
  );
    ...
    <Box flexDirection="column">
      {grid.map((row, y) => (
        <Box key={y}>
          ...
          {row.map((cell, x) => (
            <Text key={x} color={gridColors[y][x] || undefined}>
              {cell}
            </Text>
          ))}
        </Box>
      ...
      <Box>
        <Text dimColor>
          {" ".repeat(yAxisWidth)} └{"─".repeat(width)}
        </Text>
      </Box>
          ...
          <Text dimColor>
            {" ".repeat(
              Math.max(
                1,
                width -
                  data.xLabels[0].length -
                  (data.xLabels[data.xLabels.length - 1]?.length || 0)
              )
            )}
          </Text>
...
function StackedBarChart({
  data,
  maxWidth,
}: { data: StackedDataItem[]; maxWidth: number }) {
...
export default function Chart() {
  ...
  const height = Number.parseInt(args?.height || "8", 10);
